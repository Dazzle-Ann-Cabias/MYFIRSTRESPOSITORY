class Hero {
    constructor(n) {
        ...
        attack(opponent) 
        log(`${this.name} attacks ${opponent.name} with a ${this.weaponName}!`);
...
const weapon = {
    sniper: { hit: 85, miss: 15 },
    machete: { hit: 55, miss: 3 },
    bayonet: { hit: 100, miss: 65 }
};
alert("Welcome to the Two-Player Text-Based Battle Game!");
 let player1, player2;
 let currentPlayer;
...
function selectWeapon(player, weapon, imgElem) {
    ...
    messageLog(`${player === 1 ? player1.name : player2.name} selected ${weapon} as their weapon.`);
